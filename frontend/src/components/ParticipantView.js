  ...
  useEffect(() => {
    if (micRef.current) {
      if (micOn && micStream && micStream.track) {
        console.log("Updating mic stream", micStream);
        ...
        console.log("Clearing mic stream");
  ...
  useEffect(() => {
    ...
    if (videoRef.current) {
      if (webcamOn && webcamStream && webcamStream.track) {
        console.log("Updating video stream after joining", webcamStream);
        const mediaStream = new MediaStream([webcamStream.track]);
        videoRef.current.srcObject = mediaStream;
        videoRef.current.play().catch((error) => {
          console.error("Video play failed after joining:", error);
        });
      } else {
        console.log("Clearing video stream after joining");

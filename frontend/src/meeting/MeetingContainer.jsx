...
// ✅ MOVED OUTSIDE: This prevents audio reloading issues
const ParticipantMicStream = ({ participantId }) => {
...
export function MeetingContainer({ onMeetingLeave, setIsMeetingLeft }) {
  ...
  useEffect(() => {
    const debounceTimeout = setTimeout(() => {
      const remoteParticipantIds = Array.from(mMeeting.participants.keys());
      const localId = mMeeting.localParticipant?.id;

      // Only add local participant if it's not already in the list
      const participantIds =
        localId && !remoteParticipantIds.includes(localId)
  ...
  }, [mMeeting.participants, mMeeting.localParticipant]);
  ...
  usePubSub("CHAT", {
    onMessageReceived: (data) => {
      ...
      if (!isLocal) {
        ...
        audio.play().catch((error) => {
          console.log("Chat notification sound failed to play:", error);
        });
